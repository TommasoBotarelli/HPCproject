...
int main(int argc, char **argv)
{
  for (int k = 1; k < 5; k++){
    ...
    for(int n = 0; n < nt; n++) {
        ...
        sprintf(buffer, "0%d_%s", k, param.output_eta_filename);
        // output solution
        if(param.sampling_rate && !(n % param.sampling_rate)) {
        write_data_vtk(&eta, "water elevation", buffer, n);
    ...
    write_manifest_vtk("water elevation", sprintf(buffer, "0%d_%s", k, param.output_eta_filename),
