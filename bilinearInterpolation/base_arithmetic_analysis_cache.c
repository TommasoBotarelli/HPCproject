...
{
  double update_s[MAX_ITERATION];
  int n_grid_points[MAX_ITERATION];
...
double interpolate_data(const struct data *data, double x, double y)
{
  ...
  {
    val = (GET(data, i, j) * (GET_X_COORD(data, i + 1) - x) * (GET_Y_COORD(data, j + 1) - y) + GET(data, i + 1, j) * (x - GET_X_COORD(data, i)) * (GET_Y_COORD(data, j + 1) - y) + GET(data, i, j + 1) * (GET_X_COORD(data, i + 1) - x) * (y - GET_Y_COORD(data, j)) + GET(data, i + 1, j + 1) * (x - GET_X_COORD(data, i)) * (y - GET_Y_COORD(data, j))) / (data->dx * data->dy);
  }

  else if (i == data->nx - 1)
  {
    val = (GET(data, i, j) * (GET_Y_COORD(data, j + 1) - y) +
           GET(data, i, j + 1) * (y - GET_Y_COORD(data, j))) /
          data->dy;
  }
  ...
  else if (j == data->ny - 1)
  {
    val = (GET(data, i, j) * (GET_X_COORD(data, i + 1) - x) +
           GET(data, i + 1, j) * (x - GET_X_COORD(data, i))) /
          data->dx;
  }
...
void print_to_file(const struct analysis_data *analysis_data, const char *filename){
  ...
  fprintf(file, "n_points_grid;Mupdate/s;time\n");

  for (int i = 0; i < MAX_ITERATION; i++){
    fprintf(file, "%d;%f;%f;\n", analysis_data->n_grid_points[i], analysis_data->update_s[i], analysis_data->times[i]);
  }
...
int main(int argc, char **argv)
{
  ...
  struct analysis_data analysis_data;

  for (int k = 1; k < MAX_ITERATION+1; k++)
  {
    param.dx = param.dx * 2;
    param.dy = param.dy * 2;
    ...
    for (int n = 0; n < nt; n++)
    {
      ...
      for (int i = 0; i < nx; i++)
      {
        for (int j = 0; j < ny; j++)
        {
          ...
          double eta_ij = GET(&eta, i, j) - c1 / param.dx * (GET(&u, i + 1, j) - GET(&u, i, j)) - c1 / param.dy * (GET(&v, i, j + 1) - GET(&v, i, j));
          SET(&eta, i, j, eta_ij);
        }
      }
    ...
    analysis_data.n_grid_points[k-1] = eta.nx * eta.ny;
    analysis_data.update_s[k-1] = 1e-6 * (double)eta.nx * (double)eta.ny * (double)nt / time;
    analysis_data.times[k-1] = time;

    print_to_file(&analysis_data, "results.csv");
