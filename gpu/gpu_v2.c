...
int main(int argc, char **argv)
{
  ...
  struct data* p_h = &h;
  struct data* p_h_interp_u = &h_interp_u;
  struct data* p_h_interp_v = &h_interp_v;
  struct data* p_eta = &eta;
  struct data* p_u = &u;
  struct data* p_v = &v;
  ...
  #pragma omp target teams distribute parallel for \
                    ...
                    map(tofrom: p_h_interp_u[0:1], p_h_interp_u->nx, p_h_interp_u->ny, p_h_interp_u->dx, p_h_interp_u->dy, p_h_interp_u->values[0:p_h_interp_u->nx*p_h_interp_u->ny])
  for(int j = 0; j < u.ny ; j++) {
    for(int i = 0; i < u.nx + 1; i++) {
      double x = i * param.dx;
      double y = ((double)j + 0.5) * param.dy;
      double val = interpolate_data(p_h, x, y);

      SET(p_h_interp_u, i, j, val);
    }
  }
  ...
  for(int n = 0; n < nt; n++) {
    ...
    for(int j = 0; j < ny ; j++) {
      for(int i = 0; i < nx; i++) {
        ...
        double h_x_u_i1_j = GET(p_h_interp_u, i+1, j);
        double h_x_u_i_j = GET(p_h_interp_u, i, j);

        double h_x_v_i_j1 = GET(p_h_interp_v, i, j+1);
        double h_x_v_i_j = GET(p_h_interp_v, i, j);

        //Fare la divisione fuori dai for

        double eta_ij = GET(p_eta, i, j) 
                - param.dt / param.dx * (h_x_u_i1_j * GET(p_u, i+1, j) - h_x_u_i_j * GET(p_u, i, j))
                - param.dt / param.dy * (h_x_v_i_j1 * GET(p_v, i, j+1) - h_x_v_i_j * GET(p_v, i, j));
    ...
    for(int j = 0; j < ny; j++) {
      for(int i = 0; i < nx; i++) {
        double eta_ij = GET(p_eta, i, j);
        double eta_imj = GET(p_eta, (i == 0) ? 0 : i - 1, j);
        double eta_ijm = GET(p_eta, i, (j == 0) ? 0 : j - 1);
        double u_ij = one_minus_c2 * GET(p_u, i, j)
          -c1_dx * (eta_ij - eta_imj);
        double v_ij = one_minus_c2 * GET(p_v, i, j)
