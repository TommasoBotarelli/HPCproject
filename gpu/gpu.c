...
int main(int argc, char **argv)
{
  ...
  struct data* pointer_h = &h;
  struct data* pointer_h_interp_u = &h_interp_u;
  struct data* pointer_h_interp_v = &h_interp_v;
  struct data* pointer_eta = &eta;
  struct data* pointer_u = &u;
  struct data* pointer_v = &v;
  ...
  //#pragma omp target teams distribute parallel for map(to:(&h)[0:1]) map(to:h.values[0:h.nx*h.ny]) map(tofrom:(&h_interp_u)[0:1]) map(tofrom:h_interp_u.values[0:h_interp_u.nx*h_interp_u.ny]) 
  #pragma omp target teams distribute parallel for map(to:pointer_h[0:1],pointer_h->values[0:pointer_h->nx*pointer_h->ny]) map(tofrom:pointer_h_interp_u->values[0:pointer_h_interp_u->nx*pointer_h_interp_u->ny],pointer_h_interp_u[0:1])
  for(int j = 0; j < u.ny ; j++) {
    for(int i = 0; i < u.nx + 1; i++) {
      double x = i * param.dx;
      double y = ((double)j + 0.5) * param.dy;
      double val = interpolate_data(pointer_h, x, y);
      

      //h_interp_u.values[h_interp_u.nx * j + i] = val;
      SET(pointer_h_interp_u, i, j, val);
    }
  }
  ...
  #pragma omp target teams distribute parallel for map(to:pointer_h[0:1],pointer_h->values[0:pointer_h->nx*pointer_h->ny]) map(tofrom:pointer_h_interp_v->values[0:pointer_h_interp_v->nx*pointer_h_interp_v->ny],pointer_h_interp_v[0:1])
  for(int j = 0; j < v.ny +1; j++) {
    for(int i = 0; i < v.nx ; i++) {
      ...
      SET(pointer_h_interp_v, i, j, val);
      //h_interp_v.values[h_interp_v.nx * j + i] = val;
    }
  }

  double start = GET_TIME();

  //#pragma omp target data map(to:pointer_h_interp_v[0:1], pointer_h_interp_v->values[0:pointer_h_interp_v->nx*pointer_h_interp_v->ny], pointer_h_interp_u[0:1], pointer_h_interp_u->values[0:pointer_h_interp_u->nx*pointer_h_interp_u->ny]) map(tofrom:pointer_eta[0:1],pointer_u[0:1],pointer_v[0:1],pointer_eta->values[0:pointer_eta->nx*pointer_eta->ny],pointer_u->values[0:pointer_u->nx*pointer_u->ny],pointer_v->values[0:pointer_v->nx*pointer_v->ny])
  for(int n = 0; n < nt; n++) {
    ...
    // update eta
    #pragma omp target teams distribute parallel for collapse(2) map(to:pointer_h_interp_v[0:1], pointer_h_interp_v->values[0:pointer_h_interp_v->nx*pointer_h_interp_v->ny], pointer_h_interp_u[0:1], pointer_h_interp_u->values[0:pointer_h_interp_u->nx*pointer_h_interp_u->ny]) map(tofrom:pointer_eta[0:1],pointer_eta->values[0:pointer_eta->nx*pointer_eta->ny]) map(to:pointer_u[0:1], pointer_v[0:1], pointer_u->values[0:pointer_u->nx*pointer_u->ny],pointer_v->values[0:pointer_v->nx*pointer_v->ny])
    for(int j = 0; j < ny ; j++) {
      for(int i = 0; i < nx; i++) {

        double h_x_u_i1_j = GET(pointer_h_interp_u, i+1, j);
        double h_x_u_i_j = GET(pointer_h_interp_u, i, j);

        double h_x_v_i_j1 = GET(pointer_h_interp_v, i, j+1);
        double h_x_v_i_j = GET(pointer_h_interp_v, i, j);

        //Fare la divisione fuori dai for

        double eta_ij = GET(pointer_eta, i, j) 
                - param.dt / param.dx * (h_x_u_i1_j * GET(pointer_u, i+1, j) - h_x_u_i_j * GET(pointer_u, i, j))
                - param.dt / param.dy * (h_x_v_i_j1 * GET(pointer_v, i, j+1) - h_x_v_i_j * GET(pointer_v, i, j));
        
        
        SET(pointer_eta, i, j, eta_ij);
    ...
    // update u and v
    #pragma omp target teams distribute parallel for collapse(2) map(to:pointer_eta[0:1], pointer_eta->values[0:pointer_eta->nx*pointer_eta->ny]) map(tofrom:pointer_u[0:1], pointer_v[0:1], pointer_u->values[0:pointer_u->nx*pointer_u->ny],pointer_v->values[0:pointer_v->nx*pointer_v->ny])
    for(int j = 0; j < ny; j++) {
      for(int i = 0; i < nx; i++) {
        double eta_ij = GET(pointer_eta, i, j);
        double eta_imj = GET(pointer_eta, (i == 0) ? 0 : i - 1, j);
        double eta_ijm = GET(pointer_eta, i, (j == 0) ? 0 : j - 1);
        double u_ij = one_minus_c2 * GET(pointer_u, i, j)
          -c1_dx * (eta_ij - eta_imj);
        double v_ij = one_minus_c2 * GET(pointer_v, i, j)
          - c1_dy * (eta_ij - eta_ijm);
        SET(pointer_u, i, j, u_ij);
        SET(pointer_v, i, j, v_ij);
