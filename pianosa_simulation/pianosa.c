...
int read_bathymetry(struct bathymetry *data, const char *filename)
{
  FILE *fp = fopen(filename, "rb");
  if(!fp) {
    printf("Error: Could not open input data file '%s'\n", filename);
  ...
  if(ok) ok = (fread(&data->n_nan, sizeof(int), 1, fp) == 1);
  if(ok) {
    data->row_indices = (int*)malloc(data->n_nan * sizeof(int));
    if(!data->row_indices) {
      printf("Error: Could not allocate data (%d integers)\n", data->n_nan);
      ok = 0;
    }
    else {
      ok = (fread(data->row_indices, sizeof(int), data->n_nan, fp) == data->n_nan);
    }
  }
  if(ok) {
    data->column_indices = (int*)malloc(data->n_nan * sizeof(int));
    if(!data->column_indices) {
      printf("Error: Could not allocate data (%d integers)\n", data->n_nan);
      ok = 0;
    }
    else {
      ok = (fread(data->column_indices, sizeof(int), data->n_nan, fp) == data->n_nan);
    }
  }
  fclose(fp);
  if(!ok) {
...
void free_bathymetry(struct bathymetry *data)
{
  free(data->values);
  free(data->row_indices);
...
double interpolate_data(const struct data *data, double x, double y)
{
  ...
  if (i >= data->nx-1 && j >= data->ny-1)
  {
    ...
    int nan_counter = 0;
    double values[] = {NAN, NAN, NAN, NAN};

    // a sinistra in alto
    if (isnan(GET(data, i, j))){
      nan_counter++;
    }
    else {
      values[0] = GET(data, i, j);
    }
    // a destra in alto
    if (isnan(GET(data, i+1, j))){
      nan_counter++;
    }
    else {
      values[1] = GET(data, i+1, j);
    }
    // a sinistra in bass0
    if (isnan(GET(data, i, j+1))){
      nan_counter++;
    }
    else{
      values[2] = GET(data, i, j+1);
    }
    // a destra in basso
    if (isnan(GET(data, i+1, j+1))){
      nan_counter++;
    }
    else{
      values[3] = GET(data, i+1, j+1);
    }
...
int main(int argc, char **argv)
{
  ...
  struct data h_interp_u_nan;
  struct data h_interp_v_nan;
  ...
  init_data(&h_interp_u_nan, u.nx, u.ny, param.dx, param.dy, 0.);
  ...
  init_data(&h_interp_v_nan, v.nx, v.ny, param.dx, param.dy, 0.);
  ...
  for(int n = 0; n < nt; n++) {
    ...
    if(param.source_type == 1) {
      ...
      //    if (isnan(GET(&h_interp_v_nan, i, j))){
      //      SET(&v, i, j, 0.0);
      //    }  
      //  }
      //}
