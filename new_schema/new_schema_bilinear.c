...
int main(int argc, char **argv)
{
  ...
  for(int j = 0; j < 2*ny + 1; j++) {
    for(int i = 0; i < 2*nx + 1; i++) {
      double x = i * param.dx / 2;
      double y = j * param.dy / 2;
  ...
  for(int n = 0; n < nt; n++) {
    ...
    for(int i = 0; i < nx; i++) {
      for(int j = 0; j < ny ; j++) {
        ...
        double h_x_u_i1_j = GET(&h_interp, 2*(i+1), 2*j+1);
        double h_x_u_i_j = GET(&h_interp, 2*i, 2*j+1);

        double h_x_v_i_j1 = GET(&h_interp, 2*i+1, 2*(j+1));
        double h_x_v_i_j = GET(&h_interp, 2*i+1, 2*j);
