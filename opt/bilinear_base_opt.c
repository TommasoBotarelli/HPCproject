...
int main(int argc, char **argv)
{
  ...
  for(int n = 0; n < nt; n++) {
    ...
    double A = 5;
    double f = 1. / 20.;
    double initial = A * sin(2 * M_PI * f * t);
    ...
    // update eta
    double inv_dx = 1 / param.dx;
    double inv_dy = 1 / param.dy;
    ...
    for(int j = 0; j < ny ; j++) {
      for(int i = 0; i < nx; i++) {
        ...
        double eta_ij = GET(&eta, i, j)
          - c1 * inv_dx * (GET(&u, i + 1, j) - GET(&u, i, j))
          - c1 * inv_dy * (GET(&v, i, j + 1) - GET(&v, i, j));
        SET(&eta, i, j, eta_ij);
      }
    }

    // update u and v
    double c1_inv_dx = param.dt * param.g * inv_dx;
    double c1_inv_dy = param.dt * param.g * inv_dy;  
    ...
    for(int j = 0; j < ny; j++) {
      for(int i = 0; i < nx; i++) {
        ...
        double u_ij = one_minus_c2 * GET(&u, i, j)
          - c1_inv_dx * (eta_ij - eta_imj);
        double v_ij = one_minus_c2 * GET(&v, i, j)
          - c1_inv_dy * (eta_ij - eta_ijm);
        SET(&u, i, j, u_ij);
        SET(&v, i, j, v_ij);
      }
    }
